...
ggsave(filename = "results/correlation_line_plot.png", plot = correlation_line_plot, width = 10, height = 6, dpi = 300)
...
top_artists_songs <- billboard %>%
    mutate(Year = year(date)) %>%
    filter(Year >= 1910, Year <= 2014) %>%
    group_by(Year, artist, song) %>%
    summarise(Total_Weeks = sum(`weeks-on-board`, na.rm = TRUE), .groups = "drop") %>%
    arrange(Year, desc(Total_Weeks)) %>%
    group_by(Year) %>%
    slice_max(order_by = Total_Weeks, n = 10) %>%
    ungroup()

# collapse all artist and song strings for partial detection
all_artists <- paste(top_artists_songs$artist, collapse = " ")
all_songs <- paste(top_artists_songs$song, collapse = " ")
